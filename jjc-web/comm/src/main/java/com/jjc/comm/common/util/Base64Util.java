...
public class Base64Util {
    ...
    public static String rawEncode(byte[] binaryData) {
        String encoded = Base64.getEncoder().encodeToString(binaryData);
        return encoded;
    }

    public static byte[] rawDecode(String srcStr) {
        //Base64 解密
        byte[] decoded = Base64.getDecoder().decode(srcStr);
        return decoded;
    }
    ...
    public static void main(String[] args) {
        ...
        try {
           byte[] bbx =  FileUtils.toByteArray("D:\\8030WebPort\\uploadfile\\timg3.rar");
           String bas4 = Base64Util.rawEncode(bbx);
            byte[] bs = Base64Util.rawDecode(bas4);
            FileUtils.saveFile(bs,"D:\\8030WebPort\\uploadfile\\","save.rar");
        } catch (IOException e) {
            e.printStackTrace();
        }
